...
public class JiguangPush {
    ...
    private final static String appKey = "3c27b092cba4b6b367d31202";
    private final static String masterSecret = "425f4da4dc9571264023db18";
    private static JPushClient jPushClient = new JPushClient(masterSecret, appKey);
    private static boolean PRO_ENVIRONMENT = true; // TODO false表示开发，true表示生产；对android和自定义消息无意义
    ...
    public static int sendToRegistrationId(String registrationId, String notificationTitle, String msgTitle, String msgContent, Map<String, String> extrasParam) {
        int result = 0;
        try {
            PushPayload pushPayload = JiguangPush.buildPushObject_all_registrationId_alertWithTitle(registrationId, notificationTitle, msgTitle, msgContent, extrasParam);
    ...
    public static int sendToRegistrationIdQuiet(String registrationId, String notificationTitle, String msgTitle, String msgContent, Map<String, String> extrasParam) {
        int result = 0;
        try {
            PushPayload pushPayload = JiguangPush.buildPushObject_all_registrationId_alertQuiet(registrationId, notificationTitle, msgTitle, msgContent, extrasParam);
            log.info("=====>pushPayload:" + pushPayload);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            log.info("=====>pushResult:" + pushResult);
            if (pushResult.getResponseCode() == 200) {
    ...
    public static int sendToAllAndroid(String notificationTitle, String msgTitle, String msgContent, String extrasParam) {
        int result = 0;
        try {
            PushPayload pushPayload = JiguangPush.buildPushObjectAndroidAllAlertWithTitle(notificationTitle, msgTitle, msgContent, extrasParam);
            log.info("=====>pushPayload:" + pushPayload);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            log.info("=====>pushResult:" + pushResult);
            if (pushResult.getResponseCode() == 200) {
    ...
    public static int sendToAllIos(String notificationTitle, String msgTitle, String msgContent, String extrasParam) {
        int result = 0;
        try {
            PushPayload pushPayload = JiguangPush.buildPushObjectIosAllAlertWithTitle(notificationTitle, msgTitle, msgContent, extrasParam);
            log.info("=====>pushPayload:" + pushPayload);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            log.info("=====>pushResult:" + pushResult);
            if (pushResult.getResponseCode() == 200) {
    ...
    public static int sendToAll(String notificationTitle, String msgTitle, String msgContent, String extrasParam) {
        int result = 0;
        try {
            PushPayload pushPayload = JiguangPush.buildPushObjectAndroidAndIos(notificationTitle, msgTitle, msgContent, extrasParam);
            log.info("=====>pushPayload:" + pushPayload);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            log.info("=====>pushResult:" + pushResult);
            if (pushResult.getResponseCode() == 200) {
...
//                                .setSound("sound.caf")
    ...
    public static void main(String[] args) {
        ...
        JiguangPush.sendToRegistrationId("101d85590933d935524", "本地直接推送测试", "", "", map);
    }
